...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var {otu_ids, otu_labels, sample_values} = firstSample
    ...
    function labelData(data) {
      var subArray = data.slice(0, 10)
      let newArray = []
      for (var i = 0; i < subArray.length; i++) {
        newArray.push(`OTU-${subArray[i]}`)
      }
      
      return newArray.reverse()
    }

    var sortedOTU = labelData(otu_ids)
